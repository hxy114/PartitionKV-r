...
void DBImpl::MaybeScheduleFlushOrCompaction() {
  ...
  while (!is_flush_pool_empty && flush_num > 0 &&
...
bool DBImpl::SchedulePendingFlush(const FlushRequest& flush_req) {//切换wal
  mutex_.AssertHeld();
  bool enqueued = false;
...
void DBImpl::SchedulePendingCompaction(ColumnFamilyData* cfd) {//compaction完判断一次，安装superversion
  mutex_.AssertHeld();
  if (reject_new_background_jobs_) {
    return;
  }
  ...
  *reason = FlushReason::kOthers;
  ...
  while (!flush_queue_.empty()) {
    ...
    if (!error_handler_.GetBGError().ok() && error_handler_.IsBGWorkStopped() &&
        flush_reason != FlushReason::kErrorRecovery &&
        flush_reason != FlushReason::kErrorRecoveryRetryFlush) {//disable
    ...
    if (!immutable_db_options_.atomic_flush &&
        ShouldRescheduleFlushRequestToRetainUDT(flush_req)) {//disable
      assert(flush_req.cfd_to_max_mem_id_to_persist.size() == 1);
      ColumnFamilyData* cfd =
          flush_req.cfd_to_max_mem_id_to_persist.begin()->first;
  ...
  if (!bg_flush_args.empty()) {
    ...
    status = FlushMemTablesToOutputFiles(bg_flush_args, made_progress,
                                         job_context, log_buffer, thread_pri);//获取到bg_flush_args

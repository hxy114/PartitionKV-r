...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  Status PutCFImpl(uint32_t column_family_id, const Slice& key,
    ...
    PartitionIndexLayer * partitionIndexLayer = cf_mems_->GetPartitionIndexLayer();
    //ret_status =
            partitionIndexLayer->Add(sequence_, value_type, key, value, kv_prot_info,
                     concurrent_memtable_writes_, nullptr, nullptr);
    //MemTable* mem = cf_mems_->GetMemTable();
    //auto* moptions = partitionIndexLayer->GetImmutableMemTableOptions();
   ...
   /* assert(!seq_per_batch_ || !moptions->inplace_update_support);
    ...
    if (UNLIKELY(ret_status.IsTryAgain())) {
      ...
      MaybeAdvanceSeq();
      //CheckMemtableFull();//判断memtable满了吗，满了就切换
    }
    // optimize for non-recovery mode
  ...
  void CheckMemtableFull() {//TODO 需要修改
...
public:
    // cf_mems should not be shared with concurrent inserters
    PartitionInserter(SequenceNumber _sequence, ColumnFamilyMemTables* cf_mems,
                     FlushScheduler* flush_scheduler,
                     TrimHistoryScheduler* trim_history_scheduler,
                     bool ignore_missing_column_families,
                     uint64_t recovering_log_number, DB* db,
    ...
    PartitionInserter(const PartitionInserter&) = delete;
    PartitionInserter& operator=(const PartitionInserter&) = delete;

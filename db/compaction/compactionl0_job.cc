...
Status CompactionL0Job::InstallCompactionResults(
  ...
  auto manifest_wcb = [&compaction, &compaction_released](const Status& s) {
   ...
   if(status.ok()){
    auto all_start=compaction->GetSmallestUserKey(),all_limit=compaction->GetLargestUserKey();
    auto immu=compaction->input_memtable(),tmp=immu;
    auto &L0_range = compaction->column_family_data()->L0_range_;
    for(size_t i=0;i<L0_range.size();i++){
      if(L0_range[i].first==all_start&&L0_range[i].second==all_limit){
        L0_range[i]=L0_range.back();
        L0_range.pop_back();
        break;
      }
    }
    ...
    size_t imuu_list_size=compaction->memtable_size();
    auto left_father=immu->GetLeftFather(),right_father=immu->GetRightFather();
    for(size_t i=0;i<imuu_list_size;i++){
      tmp->FreePmtable();
      tmp->SetPmTableStatus(MemTable::IN_COMPACTIONED);
      tmp=tmp->next_;
    }
    if(left_father){
      if(immu->GetRole()==MemTable::immuPmtable){
        left_father->reset_immuPmtable(imuu_list_size,tmp);
      }else if(immu->GetRole()==MemTable::other_immuPmtable){
    ...
    assert(tmp== nullptr||tmp->status_==MemTable::IN_HEAD);
    if(tmp){
      if(tmp->role_==MemTable::other_immuPmtable){
        compaction->column_family_data()->top_queue_.InsertPmtable(tmp);
        tmp->status_=MemTable::IN_TOP_QUEUE;
      }
      if(tmp->role_==MemTable::immuPmtable){
        if(tmp->next_) {
          compaction->column_family_data()->top_queue_.InsertPmtable(tmp);
          tmp->status_ = MemTable::IN_TOP_QUEUE;
        }else {
          compaction->column_family_data()->low_queue_.InsertPmtable(tmp);
          tmp->status_=MemTable::IN_LOW_QUQUE;
        }
      }
